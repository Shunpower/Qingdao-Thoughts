...
设 $d_i=\prod\limits_{j=1}^{i-1}a_j$，且 $f_{i,j}$ 表示取了前 $i$ 个数并且和为 $(m \bmod d_{i+1})+j\times d_{i+1}$ 的方案数。

首先观察状态数，不难发现对于 $i$ 来说，$j$ 的上界为 $$
...
有 $n$ 个谓词 $P_{1\sim n}$，求有多少个对象 $X$ 满足 $\forall i,P_i(X)=1$。
