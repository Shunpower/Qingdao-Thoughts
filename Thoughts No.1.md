...
事实上我们容易证明左侧总是被 $d_i$ 整除的，$k\times d_{i+1}$ 和 $j\times d_i$ 都是不需要证明的，取模部分打开取模可以发现 $m$ 抵消掉，剩下 $-pd_{i+1}+qd_i$，是可以被 $d_i$ 整除的。
...
可以发现这个点是一定存在的。对于 $n-1$ 是奇数，画一下可以发现第一次向右跳 $\left\lfloor\frac{n-1}{2}\right\rfloor$ 的起点左侧显然是一个符合条件的位置；对于 $n-1$ 是偶数，画一下可以发现第一次向右跳 $\left\lfloor\frac{n-1}{2}\right\rfloor$ 的终点右侧显然是一个符合条件的位置，于是这个题就做完了。
...
显然考虑拆位，我们计算每一位被算进多少次和，然后分别加起来就行。

应用 yzc's trick，我们把 $\sum i$ 改写成 $\sum \limits_{j}\sum[i\ge j]$，然后我们就只管 $0,1$ 的事情了。

只管 $0,1$ 在这题中产生巨大优势：区间取 $\min,\max$ 将成为区间推平。

假设我们可以枚举 $j\in[0,m)$，那么所有大于 $j$ 的数全部变成 $1$，小于等于 $j$ 的数全部变成 $0$。我们只要算出此时最终的输出和就是 $j$ 这个分界点要对真正最终的输出和的贡献。

可以转成期望算，算出每个位置上 $1$ 的概率加在一起乘上总方案数，但是没有必要。

我们考虑如下观察：

- 只有对 $1$ 取 $\max$ 和对 $0$ 取 $\min$ 两种操作是有用的，剩下的操作都没用，并且只有这两种操作可以改变一个值。

假设我们现在枚举了位 $i$ 和 01 转化分界点 $j$，那么这一位上的 $1$ 被垒进和里面的总次数其实是：

令 $S_1=(2m+1)[\frac{n(n+1)}{2}-i(n-i+1)]+(m+1)i(n-i+1)$，即不会使得 $i$ 这一位上 $1$ 变成 $0$ 的一次操作可能数量。

令 $S_2=(2m+1)[\frac{n(n+1)}{2}]$，即所有可能的操作数量。

$$
\sum\limits_{k=1}^qS_2^{q-k}i(n-i+1)\sum\limits_{t=1}^{k-1}i(n-i+1)(m-j-1)S_1^{k-t-1}S_2^{t-1}
$$

套上 $i,j$ 的 $\sum$ 之后可以把 $i,j$ 项提出去，换句话说，我们只需求：

$$
\sum\limits_{k=1}^q S_2^{q-k}\sum\limits_{t=1}^{k-1} S_1^{k-t-1}S_2^{t-1}
$$

显然，求出这个之后外面的展开是 trival 的。

考虑这个东西怎么搞。容易注意到并套用等比数列求和公式：

$$
\begin{aligned}
&\sum\limits_{k=1}^q S_2^{q-k}\sum\limits_{t=1}^{k-1} S_1^{k-t-1}S_2^{t-1}\\
=&\sum\limits_{k=1}^q \frac{S_2^q}{S_2^k}\sum\limits_{t=1}^{k-1}\frac{S_1^{k}}{S_1^{t+1}}S_2^{t-1}\\
=&\sum\limits_{k=1}^q\frac{S_2^q}{S_2^k}\sum\limits_{t=1}^{k-1}\frac{S_1^k}{S_1^2}\cdot\frac{S_2^{t-1}}{S_1^{t-1}}\\
=&\sum\limits_{k=1}^q\frac{S_2^q}{S_1^2}\cdot(\frac{S_1}{S_2})^k\cdot \sum\limits_{t=1}^{k-1}(\frac{S_2}{S_1})^{t-1}\\
=&\frac{S_2^q}{S_1^2}\sum\limits_{k=1}^q(\frac{S_1}{S_2})^k\frac{(\frac{S_2}{S_1})^{k-1}-1}{\frac{S_2}{S_1}-1}\\
=&\frac{S_2^q}{S_1^2(\frac{S_2}{S_1}-1)}\sum\limits_{k=1}^q(\frac{S_1}{S_2})^k{(\frac{S_1}{S_2})^{1-k}-(\frac{S_1}{S_2})^k}\\
=&\frac{S_2^q}{S_1S_2-S_1^2}\sum\limits_{k=1}^q\frac{S_1}{S_2}-(\frac{S_1}{S_2})^k\\
=&\frac{S_2^q}{S_1S_2-S_1^2}[q\frac{S_1}{S_2}-\frac{(\frac{S_1}{S_2})^{q+1}-\frac{S_1}{S_2}}{\frac{S_1}{S_2}-1}]\\
\end{aligned}
$$

这样就能直接 $\mathcal O(\log q)$ 地取得了。令这个值为 $S'$，那么我们试图求：

$$
\sum\limits_{i=1}^n[i(n-i+1)]^2\sum\limits_{j=0}^{m-1}(m-j-1)S'
$$

直接把 $S'$ 提出来，对第二个和式应用等差数列求和，于是最终答案就是：

$$
\sum\limits_{i=1}^n[i(n-i+1)^2]\frac{m(m-1)}{2}S'
$$
...
考虑将度数不是偶数的点每两个点连边，强行将所有点的度数都变成偶数，这样也可以直接跑欧拉回路了。容易证明这样做一定能将所有奇度数点全部变成偶度数点，且两种方向的路径覆盖次数的差不超过 $1$。

注意这种连边一定得在 DFS 中完成，即对于 $u$ 的一个儿子 $v$，如果 $v$ 是奇度数点则连接 $(u,v)$，否则有 hack：
...
将所有点定向之后，直接树上差分即可。代码细节较多。

[AC Submission](https://atcoder.jp/contests/agc025/submissions/58655295)
