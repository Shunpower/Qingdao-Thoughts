...
打表找规律或者归纳法证明可以得到 $q(n)=2^{\text{popcount}(n)}-1$，于是题目可以转化为 $1\sim m$ 有多少个数的 $\text{popcount}=\text{len}(t)$ 
...
考虑 $G(m)$ 的二分图形态是，相同编号的左右部点之间都有一条边，编号互为模 $m$ 意义下逆元的左右部点且编号不同的点（即 $xy\equiv 1\pmod m\land x\ne y$ 的左部 $x$ 和右部 $y$、左部 $y$ 和右部 $x$）之间有一条边。

由于一个数不可能有两个逆元，这在最终的二分图中将体现为若干个点不相交的“叉”型结构（左部 $x$ 到右部 $y$ 和右部 $y$ 到左部 $x$ 都有边）。

考虑计数最大匹配。显然通过全选直接连接，可以得到这个二分图的最大匹配是满配。考虑放弃一些直接连接，那么空出来的点必须使用叉重新连接上。所以最大匹配的形态一定是放弃一些直接连接，增加一些“叉”，又因为“叉”互不相交所以任意选择一个“叉”的子集都一定可以调整出来。所以假设“叉”的数量是 $c$，最大匹配数量就是 $2^c$。

考虑“叉”的数量。显然我们更容易计数 $2c$，也就是“叉”的总边数。每个与 $m$ 互质的数因为存在逆元都可以贡献，但每个模 $m$ 意义下 $1$ 的二次剩余又都会贡献成重边要去掉。所以令 $f(i)$ 表示 $[1,m)$ 范围内模 $m$ 意义下 $1$ 的二次剩余数量，$2c$ 其实就是 $\varphi(m)-f(m)$。假设我们能够快速预处理出 $f(m)$，那么预处理 $c$ 和 $2^c$ 的前缀和并处理询问都是 trival 的。

令 $f(1)=1$ 后容易发现 $f$ 是一个积性函数，这使得我们可以直接线性筛。证明留给 lph。

考虑 $f$ 在质数及质数幂处的取值。根据二次剩余的性质（欧拉准则），对于任意奇质数 $p$ 都存在 $f(p)=2$。此外，容易发现对于奇质数 $p$ 的幂次 $p^k$ 处也都存在 $f(p^k)=2$。对于 $2$，首先存在 $f(2)=1,f(4)=2,f(8)=4$，对于 $2^k,k\ge 3$，都存在 $f(2^k)=4$。证明留给 lph。
